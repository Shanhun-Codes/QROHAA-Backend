...
describe('Agents resource', () => {
  ...
  it('creates an agent with normalized colors and active default questions', async () => {
    ...
    await service.create({ firstName: 'Ava', lastName: 'Agent', email: 'ava@example.com', phone: '+14155552671', primaryColor: 'ffffff' } as any);
    expect(transaction.agent.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ primaryColor: '#ffffff', agentFeedbackQuestions: { create: [{ questionId: 'question-1', sortOrder: 0 }] } }) }));
  });

  it('rejects duplicate feedback-question selections', async () => {
    prisma.agent.findUnique.mockResolvedValue({ id: 'agent-1' });
    await expect(service.replaceFeedbackQuestions('agent-1', [{ questionId: 'question-1', required: false, sortOrder: 0 }, { questionId: 'question-1', required: true, sortOrder: 1 }])).rejects.toBeInstanceOf(BadRequestException);
  });
