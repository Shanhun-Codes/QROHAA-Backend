...
describe('FeedbackQuestions resource', () => {
  ...
  it('creates questions with their ordered options', () => {
    ...
    service.create(dto);
    expect(prisma.feedbackQuestion.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ options: { createMany: { data: dto.options } } }) }));
  });

  it('lists questions and throws for an unknown question ID', async () => {
    service.findAll();
    expect(prisma.feedbackQuestion.findMany).toHaveBeenCalledWith(expect.objectContaining({ orderBy: { key: 'asc' } }));
  ...
  it('replaces options transactionally on update and forwards controller IDs unchanged', async () => {
    ...
    expect(transaction.feedbackQuestionOption.deleteMany).toHaveBeenCalledWith({ where: { questionId: 'question-1' } });
    jest.spyOn(service, 'findOne').mockResolvedValue({ id: 'question-1' } as any);
