...
describe('OpenHouse resource', () => {
  ...
  const dto = { agentId: 'agent-1', propertyId: 'property-1', startsAt: new Date(), endsAt: new Date() };

  beforeEach(() => { jest.clearAllMocks(); prisma.openHouse.findUnique.mockResolvedValue(null); });

  it('snapshots active agent questions when creating an open house', async () => {
    prisma.agentFeedbackQuestion.findMany.mockResolvedValue([{ questionId: 'question-1', required: true, sortOrder: 0 }]);
    transaction.openHouse.create.mockResolvedValue({ id: 'open-house-1' });
    await service.create(dto);
    expect(transaction.openHouse.create).toHaveBeenCalledWith(expect.objectContaining({
      data: expect.objectContaining({ openHouseFeedbackQuestions: { createMany: { data: [{ questionId: 'question-1', required: true, sortOrder: 0 }] } } }),
    }));
  });
  ...
  it('lists newest open houses and forwards controller creation', () => {
    service.findAll();
    expect(prisma.openHouse.findMany).toHaveBeenCalledWith({ orderBy: { createdAt: 'desc' } });
    jest.spyOn(service, 'create').mockResolvedValue({ id: 'open-house-1' } as any);
